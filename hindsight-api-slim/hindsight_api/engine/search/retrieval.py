...
async def retrieve_semantic_bm25_combined(
    ...
    cols = (
        ...
        "fact_type, document_id, chunk_id, tags, metadata, proof_count, decayed_quality"
    )
...
async def retrieve_temporal_combined(
    ...
    entry_points = await conn.fetch(
        ...
        sim_ranked AS (
            SELECT mu.id, mu.text, mu.context, mu.event_date, mu.occurred_start, mu.occurred_end, mu.mentioned_at, mu.fact_type, mu.proof_count, mu.document_id, mu.chunk_id, mu.tags, mu.metadata, mu.decayed_quality,
        ...
        SELECT id, text, context, event_date, occurred_start, occurred_end, mentioned_at, fact_type, proof_count, document_id, chunk_id, tags, metadata, decayed_quality, similarity
        FROM sim_ranked
        WHERE sim_rn <= 10
        """,
        *params,
